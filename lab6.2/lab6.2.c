...
//EVERYTHING BELOW IS NOT NEEDED FOR EXAMIFY. USE THIS CODE TO TEST.

void printSolved(int finalSud[4][4]) //prints solved sudoko
{
    printf("\n");
    for (int i = 0; i < 4; i++)
    {
...
int main ()
{
    int sud[4][4] = {{0, 3, 4, 0}, {4, 0, 0, 2}, {1, 0, 0, 3}, {0, 2, 1, 0}}; //sudoko to be solved
    for (int i = 0; i < 4; i++) //this for loop prints the unsolved sudoko
    {
        for (int j = 0; j < 4; j++)
        {
            printf("%d ", sud[i][j]);
    ...
    fillSudoko(4, sud); //call the sudoko solver function 
    printSolved(sud); //call function to print solved sudoko
}
